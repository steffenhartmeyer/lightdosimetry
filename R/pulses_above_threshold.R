...
pulses_above_threshold = function(lightVar,
  ...
  if(is.numeric(min_length)) minlen = min_length
  else minlen = parse.timeunit.tosecs(min_length)$secs / sampling_int

  # Parse maximum number of interruptions
  if(is.numeric(max_interrupt)) maxint = max_interrupt
  else maxint = parse.timeunit.tosecs(max_interrupt)$secs / sampling_int
  ...
  for(c in threshold){
    ...
    p_len = (unlist(lapply(clusters.light, length)) * sampling_int) %>%
      from.secs(unit_pulse_length) %>% mean()
    p_time = p_n * p_len
